...
import { conflictWarning, strategyDiscardsReview } from '../importConflict'
import { jobClick, panelState } from '../importPanel'
import { errorText, api, post } from '../api'
import { handoffOriginLabel, handoffOutcome, handoffRequest, type HandoffOutcome } from '../handoff'
...
export default function ImportPage({ aiEnabled, currentWeekStart, route, notify }: { aiEnabled: boolean; currentWeekStart: string; route?: Record<string, string>; notify: (message: string, kind?: 'success' | 'error') => void }) {
  ...
  // 다른 서비스에서 넘겨받기. The route carries a source and a one-time claim;
  // the claim is spent on the first attempt, so the address is rewritten
  // without it before the request goes out and the outcome stays on the
  // screen rather than passing as a toast.
  const [handoff, setHandoff] = useState<HandoffOutcome>()
  ...
  useEffect(() => { loadHistory().catch(() => setJobsFailed(true)) }, [])
  useEffect(() => {
    const request = handoffRequest(route)
    if (!request || handoffStarted.current) return
    handoffStarted.current = true
    replaceRoute('import')
    setHandoff(handoffOutcome('receiving', request.source))
    setBusy(true)
    post<{ id: number; filename: string }>('/api/v1/handoff/receive', request)
      .then(async result => { setHandoff(handoffOutcome('received', request.source, result.filename)); setSelectedID(result.id); await Promise.all([loadHistory(), loadDetail(result.id)]) })
      .catch(error => setHandoff(handoffOutcome('failed', request.source, errorText(error, '문서를 받아 오지 못했습니다.'))))
  ...
  useEffect(() => { if (selectedID) { setDetailFailed(false); loadDetail(selectedID).catch(() => setDetailFailed(true)) } }, [selectedID])
  ...
  return <><PageHeader title="과거 PPTX 가져오기" description="기존 주간보고를 OpenXML 파서와 AI로 분석한 뒤 검토·확정하여 과거 데이터로 적재합니다."/>
    {handoff && <Card title="다른 서비스에서 넘겨받기"><p className={handoff.state === 'failed' ? 'import-error' : handoff.state === 'received' ? 'import-linked' : 'muted'} role="status">{handoff.text}</p></Card>}
