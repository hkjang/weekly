...
function SettingsTab({ notify, changed }: { notify: (message: string, kind?: 'success' | 'error') => void; changed: () => Promise<void> }) {
  ...
  const booleanKeys = ['workflow.enabled','auth.local_enabled','oidc.enabled','oidc.auto_provision','ai.enabled','confluence.enabled','confluence.ai_enabled','confluence.analyze_body','confluence.include_blogs','confluence.auto_map_email_localpart','confluence.auto_map_username','ai.embedding_enabled','itsm.enabled']
