...
import type { AdminUser, AdminUserPage, ConfluenceMapping, ConfluenceSyncStatus, EmbeddingStatus, EncryptionStatus, Organization, Setting, MailHealth, TrackingViolation } from '../types'
...
function SettingsTab({ notify, changed }: { notify: (message: string, kind?: 'success' | 'error') => void; changed: () => Promise<void> }) {
  ...
  const [itsmSample, setItsmSample] = useState('')
  // The administrator setting this up is debugging a URL template, a header and
  // a path into somebody else's JSON. A pass/fail would make that a guessing
  // game, so the answer carries the status code and what came back.
  const testITSM = async () => {
    try {
      const result = await post<{ ok: boolean; title?: string; url?: string; lookupUrl?: string; status?: number; snippet?: string; error?: string }>(
        '/api/v1/admin/settings/itsm/test', { id: itsmSample.trim() })
      if (result.ok) notify(`제목: ${result.title} · 이동 주소: ${result.url || '미설정'}`)
      else notify(`${result.error ?? '조회 실패'}${result.status ? ` (HTTP ${result.status})` : ''}${result.snippet ? ` · 응답: ${result.snippet.slice(0, 120)}` : ''}`, 'error')
    } catch (error) { notify(errorText(error, 'ITSM 조회 시험에 실패했습니다.'), 'error') }
  }
  // What the browser refused while a tracking snippet was on. Loaded beside
  // the settings that cause it, because the console error it replaces is the
  // one an administrator pasting a snippet never gets to see.
  const [violations, setViolations] = useState<TrackingViolation[]>()
  const loadViolations = () => api<{ items: TrackingViolation[] }>('/api/v1/admin/tracking/violations').then(result => setViolations(result.items)).catch(() => setViolations(undefined))
  useEffect(() => { loadViolations() }, [])
  const allowOrigin = async (origin: string) => { try { await post('/api/v1/admin/tracking/allow', { origin }); await Promise.all([load(), loadViolations()]); notify(`${origin} 을 허용 목록에 더했습니다. 화면을 새로 열면 정책에 반영됩니다.`) } catch (error) { notify(errorText(error, '출처를 허용할 수 없습니다.'), 'error') } }
  const clearViolations = async () => { try { await del('/api/v1/admin/tracking/violations'); await loadViolations(); notify('막힌 출처 목록을 비웠습니다. 화면을 다시 열어 아직 막히는 것이 있는지 보십시오.') } catch (error) { notify(errorText(error, '목록을 비울 수 없습니다.'), 'error') } }
