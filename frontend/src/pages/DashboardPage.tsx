...
import type { AnalyticsOverview, Report, SessionInfo } from '../types'
...
export default function DashboardPage({ session, navigate }: { session: SessionInfo; navigate: (page: 'current' | 'team') => void }) {
  ...
  useEffect(() => { api<Report | null>('/api/v1/reports/current').then(setReport); if (session.user.role !== 'USER') api<AnalyticsOverview>('/api/v1/analytics/overview').then(setAnalytics) }, [session.user.role])
