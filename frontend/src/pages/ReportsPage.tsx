...
import { Modal, Button, Card, Empty, PageHeader, SourceBadge, Spinner, StatusBadge, formatDate } from '../components'
...
export default function ReportsPage({ currentWeekStart, openReportId, notify }: {
  ...
  const confirmDiscardEdits = () => {
    ...
    const changed = summary !== selected.summary
    ...
    return !changed || confirm('수정한 내용을 저장하지 않고 닫으시겠습니까?')
  }
  ...
  return <><PageHeader title="과거 보고" description="이전 주간보고를 조회하고 복제·수정·삭제하거나 PPTX로 내보냅니다." action={<select value={status} onChange={e => setStatus(e.target.value)}><option value="">전체 상태</option><option value="DRAFT">작성 중</option><option value="SUBMITTED">검토 대기</option><option value="REVISION_REQUESTED">반려</option><option value="APPROVED">승인</option><option value="CLOSED">확정</option></select>} />
    {!reports ? <Spinner/> : !reports.length ? <Empty action={<Button onClick={() => { window.location.hash = '#/current' }}>이번 주 보고서 작성</Button>}>아직 저장한 보고서가 없습니다. 이번 주 보고서를 작성하면 여기에 쌓입니다.</Empty> : <Card><div className="table-wrap"><table><thead><tr><th>주차</th><th>작성자</th><th>요약</th><th>출처</th><th>상태</th><th>수정일</th><th/></tr></thead><tbody>{reports.map(report => <tr key={report.id} onClick={() => open(report.id)}><td>{report.weekStart}</td><td>{report.displayName}</td><td className="truncate">{report.summary || '-'}</td><td><SourceBadge source={report.sourceType}/></td><td><StatusBadge status={report.status}/></td><td>{formatDate(report.updatedAt)}</td><td><button className="icon-button" onClick={e => { e.stopPropagation(); void saveExport(report.id) }} title="PPTX 다운로드">↓</button></td></tr>)}</tbody></table></div>
