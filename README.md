...
```bash
sha256sum weekly-v0.245.0.tar.gz
gzip -dc weekly-v0.245.0.tar.gz | tar -xO manifest.json | grep -o 'blobs/sha256/[0-9a-f]\{64\}' | head -1
gzip -dc weekly-v0.245.0.tar.gz | docker load
...
`go test` 가 잡지 못하는 것을 잡는 검사가 열한 개 있습니다 — 자기 대상을 실행하지 않는 시험, 통과하지만 무엇도 지키지 않는 시험, 규모에서만 보이는 응답 크기, 월요일 아침의 동시성. 무엇을 언제 돌리는지는 [`docs/CHECKS.md`](docs/CHECKS.md) 에 있습니다.
