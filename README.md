...
- 회의에서 다룰 것만 모으고 전체 화면 발표 모드로 바로 진행하는 회의 모드
...
```bash
gzip -dc weekly-v0.13.0.tar.gz | docker load
...
회의 화면에서 `발표 모드`를 누르면 안건이 전체 화면 슬라이드로 바뀝니다. 한 슬라이드에 한 안건만 담아 회의실에서 읽을 수 있는 크기로 표시합니다.
