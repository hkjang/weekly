...
gzip -dc weekly-v0.294.0.tar.gz | docker load
...
OIDC가 켜진 배포에서 Weekly 세션 없이 화면을 처음 열면 브라우저는 Keycloak에 `prompt=none`으로 한 번만 확인합니다. 이미 Keycloak에 로그인돼 있으면 로그인 화면을 거치지 않고 원래 열려던 `#/...` 화면으로 돌아오며, Keycloak 세션이 없거나 사용자 상호작용이 필요하면 평소 Weekly 로그인 화면에 머뭅니다. 이 확인은 앱을 처음 여는 시점의 401에만 적용됩니다. 작성 중 세션이 만료된 탭을 다른 사이트로 보내 내용을 잃게 하지는 않습니다.
