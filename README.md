...
- 팀장·조직장·관리자가 고른 요일 오전 9시에 아직 제출하지 않은 담당 범위의 활성 사용자에게 주차당 한 번 보내는 작성 권고 메일
...
```bash
sha256sum weekly-v0.278.0.tar.gz
gzip -dc weekly-v0.278.0.tar.gz | tar -xO manifest.json | grep -o 'blobs/sha256/[0-9a-f]\{64\}' | head -1
gzip -dc weekly-v0.278.0.tar.gz | docker load
...
- 같은 사용자를 여러 팀장·조직장이 함께 담당해도 한 주에 메일 한 통만 큐에 넣습니다. 발송 차례 전에 사용자가 제출했거나 권고자의 역할·조직·설정이 바뀌면 오래된 큐는 버립니다.
- SMTP가 선택 시각에 준비되지 않았더라도 현재 주 안에 설정이 정상화되면 다음 확인에서 따라잡습니다. 과거 주차의 권고 메일을 소급해 쌓지는 않습니다.
