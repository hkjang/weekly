...
var settingDefinitions = map[string]settingDefinition{
	...
	// Which services this one accepts handoff claims from. Empty by default,
	// and empty means the feature does not exist: a source outside this list
	// is refused before any request is made, because the alternative is a
	// service that fetches any address on the intranet on request.
	"handoff.allowed_origins":             {Validate: validOriginList},
