...
func TestPrepareClonedReportStructureModeResetsWeeklyContent(t *testing.T) {
	workItemID := int64(41)
	source := []reportItem{{ID: 9, WorkItemID: &workItemID, Category: "개발", Title: "복제 기능", CurrentResult: "완료", NextPlan: "배포", Issue: "없음", ManagementAsk: "검토자 지정", Progress: 80, SortOrder: 7}}
	...
	item := items[0]
	if item.ID != 0 || item.WorkItemID == nil || *item.WorkItemID != workItemID || item.Category != "개발" || item.Title != "복제 기능" || item.CurrentResult != "" || item.NextPlan != "" || item.Issue != "" || item.ManagementAsk != "" || item.Progress != 0 || item.SortOrder != 0 {
		t.Fatalf("structure clone retained stale weekly data: %+v", item)
	}
}

func TestPrepareClonedReportFullModeCopiesContentWithoutReportItemIdentity(t *testing.T) {
	workItemID := int64(41)
	source := []reportItem{{ID: 9, WorkItemID: &workItemID, Category: "개발", Title: "복제 기능", CurrentResult: "완료", NextPlan: "배포", Issue: "없음", ManagementAsk: "검토자 지정", Progress: 80, SortOrder: 4}}
	...
	item := items[0]
	if item.ID != 0 || item.WorkItemID == nil || *item.WorkItemID != workItemID || item.CurrentResult != "완료" || item.NextPlan != "배포" || item.Issue != "없음" || item.ManagementAsk != "검토자 지정" || item.Progress != 80 || item.SortOrder != 0 {
