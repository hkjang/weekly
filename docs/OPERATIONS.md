...
- 감사 이력은 관리자 화면에서 작업·행위자·기간으로 조회하고 넘겨 볼 수 있다. `audit.retention_days`(기본 365일, 0이면 무기한)가 지난 기록은 유지보수 주기에 삭제한다
...
**주차 시작 요일은 사실상 데이터 이관이다.** 저장된 보고서는 작성 당시의 주차 시작일을 그대로 갖고 있고 모든 파생 지표가 그 격자로 묶이므로, 요일을 바꾸면 기존 보고서가 격자에서 벗어난다. 벗어난 보고서는 참여 분석에서 빠지고, 기간 보고에서 한 주가 두 주로 집계될 수 있다. 전환 주에는 기존 보고서와 기간이 겹쳐 새 보고서를 만들 수 없으므로 작성자는 기존 보고서를 열어 이어 써야 한다. 서비스는 기존 보고서가 있으면 영향 범위를 알리고 확인을 요구하며, 강행하면 경고 로그를 남긴다. 운영 중 변경은 피하고, 꼭 필요하면 분기 경계처럼 영향이 적은 시점에 한다.
