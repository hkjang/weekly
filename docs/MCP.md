...
받은 것이 곧 전부입니다. 그래서 업무는 최대 100건이고, 그 위에 **64 KiB 상한**이 함께
겁니다. 행 수만으로는 크기가 잡히지 않기 때문입니다 — 같은 100건이 한 달이면 108 KB,
...
문장으로 적습니다. `itemsTotal`과 `timelineItems`가 그 숫자를 함께 담습니다.
