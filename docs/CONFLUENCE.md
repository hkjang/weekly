...
flowchart LR
  ...
  V -->|일치| A[제목+본문 AI 업무 판별·제목 정규화·병합]
  V -->|일치| S[AI 원자 사실 요약]
  V -->|불일치| N[진단 기록·다음 Sync 재처리]
  S --> G[evidencePageIds 입력 집합 검증]
  G --> P[(Report Candidate)]
  P --> U[사용자 확인·수정·제외·보고서 반영]
```

Worker는 마지막 성공 시각에 2분 중첩 구간을 더해 변경 누락을 방지한다. Page ID와 Version, 사용자, 주차를 이용해 같은 버전의 재분석을 AI 호출 전에 차단한다. 검색 결과의 Page ID·Version과 뒤이어 조회한 본문의 Page ID·Version도 비교한다. 사용자가 그 사이 문서를 수정해 값이 달라지면 `BODY_VERSION_CHANGED` 진단을 남기고 이전 메타데이터에 새 본문을 연결하지 않으며, 다음 중첩 Sync에서 완성된 새 버전을 다시 처리한다. AI 분류는 입력된 모든 Page ID를 정확히 한 번씩 포함해야 하며 하나라도 누락되면 해당 응답 전체를 버리고 결정적 후보 생성으로 전환한다. 따라서 불완전한 모델 응답이나 동시 수정 때문에 증분 기준시각만 전진하고 Page가 조용히 유실되지 않는다. 여러 서비스 복제본이 동시에 기동해도 PostgreSQL Advisory Lock으로 하나의 Worker만 수집한다.
...
`hong@example.com`은 `hong`으로 자동 매핑된다. 한 외부 아이디가 여러 사용자와 일치하거나 한 사용자가 이미 다른 Confluence 아이디에 연결된 경우 자동 확정하지 않는다. 비활성화한 명시 매핑도 자동으로 되살리지 않는다. 관리자는 `Confluence 자동화` 탭에서 추천값, 매핑 근거와 미매핑 사용자를 확인한다.
...
업무 키워드, 개인 Space 접두사와 작성·수정·회의·공지·휴가 등 점수도 같은 화면에서 조정한다. AI를 사용할 때는 공통 AI Gateway 설정의 Structured Output 지원 모델을 사용한다. 요약 모델은 금주 실적·차주 계획·이슈를 자유 문자열이 아닌 독립 사실 배열로 반환한다. 각 사실은 `kind`, 한 가지 사실만 담은 `text`, 비어 있지 않은 `evidencePageIds`, `confidence`를 포함하며 서버는 모든 근거 ID가 해당 호출의 입력 Page 집합에 속하는지 검증한 뒤 한 행씩 투영한다. 잘못된 종류·빈 사실·알 수 없는 근거 ID에는 검증 사유를 포함해 한 번만 교정을 요청한다. AI 장애, 응답 구조 오류 또는 교정 후 재실패 시에는 `AI_SUMMARY` 진단을 남기고 검토 최소 점수 이상인 제목을 보수적인 원본 제목 후보로 유지한다.
...
`body.storage`는 메모리에만 머문다. script/style과 XML 태그를 제거하되 제목, 문단, 목록, 표 행·표 셀과 코드 블록 경계를 줄바꿈으로 보존하고 HTML Entity를 해제한 뒤 크기를 제한한다. 한도를 넘으면 시작과 끝 문맥을 남기고 중간 생략 marker를 넣는다. 규칙 점수를 통과한 Page는 이 제한된 미리보기를 제목·Space와 함께 업무 판정에 사용하므로 일반적인 제목의 문서도 실제 본문으로 판단할 수 있다. `evidencePageIds` 검증은 각 근거 ID가 모델에 제공된 Page인지 확인하는 출처 무결성 검증이며, 사실 문장의 의미가 본문과 일치하는지는 사용자가 후보의 출처 링크에서 최종 검토해야 한다. 원문은 DB, 감사 로그, 일반 로그에 저장하지 않는다. 외부 AI로 사내 문서를 전달할 수 없는 환경에서는 반드시 사내망 AI Gateway를 사용하거나 본문 분석을 끈다.
...
| Metadata/본문 Page ID·Version 불일치 | `BODY_VERSION_CHANGED` 진단 후 이번 회차 제외, 다음 Sync 재처리 |
| 429/5xx/네트워크 | 제한된 지수형 재시도 후 진단 기록 |
| AI 오류·입력 Page 누락·근거 ID 오류 | 1회 교정 후 결정적 제목 후보와 출처 유지, 원문 미저장 |
