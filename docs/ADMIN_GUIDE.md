...
|---|---|---|---|
| 지난주 전체 자동 복제 | 새 보고 주차, 서비스 시간대와 `workflow.week_start` 기준 | 사용자별 주차당 한 번. 이번 주와 날짜가 겹치는 보고서가 있으면 건너뜀. 원본도 지난주와 날짜가 겹치는 보고서로 찾으므로 `workflow.week_start`를 바꾼 다음 주에도 복제가 끊기지 않음(v0.283.0 이전에는 정확일치로 물어 한 주를 건너뜀) | 없음 |
