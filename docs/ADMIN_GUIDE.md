...
요일을 바꾸면 주 격자만 옮겨지고 이미 저장된 보고서의 `week_start`는 그대로 남습니다. 전환되는 한 주 동안 작성 화면은 이번 7일을 덮고 있는 기존 보고서를 열어 작성자가 이어 쓰게 합니다(v0.282.0부터. 그 이전에는 빈 편집기가 떴고 저장이 `REPORT_PERIOD_OVERLAPS`로 거절되었습니다). 같은 기간의 두 번째 보고서는 종전과 같이 만들 수 없습니다.
