...
| 서비스 간 문서 넘기기 | 문서를 넘겨받을 서비스 오리진(쉼표 구분) | 기본값은 빈 목록 = 기능 없음(3.10 절) |
...
### 3.10 서비스 간 문서 넘기기 (허용 목록)

다른 사내 서비스(예: `ptium`)가 만든 PPTX 를 사람이 내려받아 다시 올리지 않고 넘겨받습니다. 보내는 쪽이 **한 번만 쓸 수 있는 5분짜리 표(claim)** 를 발급해 브라우저를 `https://weekly.intra/handoff?source=https://ptium.intra&claim=…` 으로 보내면, Weekly 는 로그인한 사람의 이름으로 `source` 에서 직접 파일을 받아 와 `과거 PPTX 가져오기` 의 Import 작업으로 올립니다. 서비스끼리 서로의 자격 증명을 들고 있지 않습니다 — 표가 곧 자격이고, 한 번 받아 가면 끝입니다.

**설정 방법**

1. `관리자 설정 → 서비스 설정 → 서비스 간 문서 넘기기` 카드를 엽니다.
2. `문서를 넘겨받을 서비스 오리진` 에 보내는 서비스의 오리진을 적습니다. 쉼표나 줄바꿈으로 여러 개를 적을 수 있습니다. 예: `https://ptium.intra, https://muni.intra:8443`
3. **오리진만** 적습니다 — 스킴·호스트·포트까지. 경로(`/handoff`), 쿼리, 사용자 정보가 붙은 줄은 저장이 거절됩니다. 대소문자와 끝 슬래시는 같은 것으로 봅니다.
4. `설정 저장`. 이 기능은 `AI 작성·PPTX 분석 사용` 이 켜져 있어야 동작합니다(넘겨받은 PPTX 도 같은 분석 파이프라인을 탑니다).

| 설정 | 기본값 | 의미 |
|---|---|---|
| `handoff.allowed_origins` | (빈 값) | 표를 받아 올 수 있는 오리진 목록. **비어 있으면 아무것도 받지 않습니다** — 새로 설치한 곳에서는 달라지는 것이 없습니다 |
| `import.max_file_mb` | 25 | 넘겨받는 파일 하나의 상한. 넘으면 중간에 끊습니다 |

**왜 허용 목록인가.** `source` 는 밖에서 들어온 값입니다. 그대로 받아 오면 Weekly 가 로그인한 아무나의 부탁으로 **사내 아무 주소나 대신 긁어 오는 도구**가 됩니다. 그래서 목록에 없는 오리진은 요청을 보내지 않고 거절하고, 리다이렉트는 따라가지 않으며(다른 주소로 안내하는 응답은 실패로 답합니다), 응답의 `Content-Type` 이 PPTX 가 아니면 본문을 읽지 않고 버리고, 크기와 시간(30초)에 상한을 둡니다.

**출처.** 넘겨받은 파일은 Import 화면에 `출처 ptium.intra 에서 넘겨받음` 으로 표시되고, 감사 로그 `handoff.receive` 에 오리진·파일명·크기가 남습니다. 표 자체는 감사 로그·서버 로그·응답 어디에도 적지 않습니다.

**실패했을 때 사람이 보는 문장**

| 문장 | 뜻 |
|---|---|
| 관리자가 허용한 서비스에서만 문서를 넘겨받을 수 있습니다 | `source` 가 목록에 없음. 이 카드에 오리진을 더하십시오 |
| 표가 만료됐거나 이미 쓰였습니다 | 보내는 쪽의 404. 5분이 지났거나 같은 표를 두 번 열었습니다 — 보내는 쪽에서 다시 보내면 됩니다 |
| 이 서비스는 PPTX 만 넘겨받을 수 있습니다 | 보내는 쪽이 마크다운 등 다른 형식을 보냈습니다 |
| 넘겨받을 파일이 … 크기 제한을 넘습니다 | `PPTX 파일당 최대 MB` 를 올리거나 파일을 나누십시오 |
| 보내는 서비스가 다른 주소로 안내했습니다 | 원본이 리다이렉트로 답했습니다. 허용 목록에 최종 주소의 오리진을 적고, 보내는 쪽이 그 주소로 직접 발급하게 하십시오 |
...
- `서비스 간 문서 넘기기` 의 허용 목록은 비어 있는 것이 기본입니다. 실제로 문서를 보내는 서비스의 오리진만 적고, 와일드카드나 경로가 있는 주소는 받지 않습니다(3.10 절).
