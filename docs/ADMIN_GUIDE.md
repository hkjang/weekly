...
| 방문 추적 | 사용(기본 꺼짐), 추적 도구(Momento·GA4·GTM·Matomo·직접 붙여 넣기), Momento 주소·사이트 ID·연결 경로, 측정 ID, Matomo 주소·사이트 ID, 붙여 넣는 코드(8KB), 추가 허용 출처, 삽입 위치 | 브라우저가 막은 출처 목록과 `허용` 단추가 카드 아래에 있습니다(3.10 절) |
...
### 3.10 방문 추적 스크립트와 CSP

어느 화면이 실제로 쓰이는지를 재려면 추적 스크립트가 화면에 실려야 합니다. `방문 추적` 카드에서 도구를 고르고 `방문 추적 사용`을 켠 뒤 `설정 저장`을 누르면, 그다음부터 열리는 화면(`index.html`)에 그 도구의 `<script>`가 들어갑니다. **기본은 꺼짐**이고, 새로 설치한 곳은 켜기 전까지 화면도 정책도 이전 판과 바이트 단위로 같습니다.

| 설정 | 무엇 |
|---|---|
| `방문 추적 사용` | 켜야 붙습니다. 도구에 필요한 값이 비어 있으면 저장이 거부되고 무엇이 비었는지 말합니다 |
| `추적 도구` | `Momento (사내 수집기)` · `Google Analytics 4` · `Google Tag Manager` · `Matomo` · `직접 붙여 넣기`. **Momento 가 첫 자리**인 이유는 데이터가 사내를 벗어나지 않는 유일한 선택지이기 때문입니다 |
| `Momento 수집기 주소` / `Momento 사이트 ID` | 수집기의 `https://…` 주소와 관리 콘솔에서 만든 사이트 ID |
| `Momento 연결 경로` | `이 서버를 거침 (/momento/*, 권장)` 이면 Weekly 가 `/momento/*` 를 수집기로 넘기고 스니펫에 `data-endpoint="/momento"` 를 줍니다 — 브라우저가 보기에 모든 요청이 같은 오리진이라 정책에 바깥 출처가 하나도 들어가지 않습니다. `수집기 주소로 직접` 은 수집기 오리진을 정책에 더합니다 |
| `GA4 측정 ID · GTM 컨테이너 ID` | `G-…` 또는 `GTM-…` |
| `Matomo 주소` / `Matomo 사이트 ID` | Matomo 의 주소와 사이트 번호 |
| `직접 붙여 넣는 추적 코드` | 도구가 준 스니펫 그대로. **8KB** 를 넘으면 저장되지 않습니다 |
| `추가로 허용할 출처` | 스니펫에서 자동으로 읽지 못한 출처를 손으로 더하는 자리. 아래의 `허용` 단추가 여기에 씁니다 |
| `삽입 위치` | `head`(기본) 또는 `body` 의 닫는 태그 바로 앞 |

**왜 `<script>` 한 줄이 아닌가 — 콘텐츠 보안 정책.** Weekly 의 모든 화면은 `Content-Security-Policy: script-src 'self'` 로 잠겨 있습니다. 이 정책 아래에서 인라인 스크립트나 바깥 주소의 스크립트는 브라우저가 **조용히** 막습니다 — 화면은 멀쩡하고 콘솔에만 한 줄 남으며, 수집기에는 아무것도 도착하지 않습니다. 그래서 Weekly 는 다음을 함께 합니다.

1. 화면을 낼 때마다 128비트 **nonce** 를 만들어 스니펫의 모든 `<script>` 태그에 `nonce="…"` 를 붙이고(이미 있는 태그는 그대로), 같은 값을 그 응답의 `script-src` 에 `'nonce-…'` 로 넣습니다. 요청마다 다르므로 남이 가로채 쓸 수 없습니다.
2. 도구가 불러오고 보내는 출처를 정책의 `script-src` · `connect-src` · `img-src` 에 더합니다. GA4·GTM·Matomo 는 도구가 정해져 있고, 직접 붙여 넣은 코드에서는 본문의 `http(s)://…` 주소를 읽어 출처로 씁니다.
3. 추적이 켜져 있는 동안에만 정책에 `report-uri /api/v1/tracking/csp-report` 를 넣어, 그래도 막힌 것을 브라우저가 신고하게 합니다. 신고는 **출처와 지시어**만 메모리의 짧은 목록(서로 다른 출처 100개)에 남고, 카드 아래 `브라우저가 막은 출처` 표에 나타납니다. 한 줄의 `허용` 을 누르면 `추가로 허용할 출처` 에 그 출처가 더해지고, 다음에 여는 화면부터 정책에 들어갑니다. 서버를 다시 시작하면 목록은 비워집니다.

`'unsafe-inline'` 으로 정책을 푸는 일은 **하지 않습니다**. 한 번 풀면 그 앱의 모든 인라인 스크립트가 함께 허용되고, 추적을 끈 뒤에도 정책은 느슨한 채 남기 때문입니다. `방문 추적 사용` 을 끄면 정책은 원래의 `script-src 'self'` 로 돌아가고 `/momento/*` 는 404 로 닫힙니다.

**붙지 않는 곳.** `/api/*` · `/mcp` · `/healthz` · `/readyz` · `/momento/*` 의 응답은 화면이 아니므로 스니펫이 없고, 정책도 더 좁습니다(`default-src 'none'`). 로그인 화면은 같은 `index.html` 이라 스니펫이 실리지만 스니펫은 화면의 입력값을 읽지 않으며, Momento 는 URL 의 질의 문자열과 fragment 를 지우고 보냅니다. Weekly 는 화면을 해시 주소(`#/admin` 등) 로 구분하는 단일 페이지 앱이라 서버가 관리 화면만 골라 뺄 수 없어, 관리 화면을 제외하는 선택지는 두지 않았습니다 — 관리자의 방문도 함께 셉니다.

**확인하는 법.** 켜고 화면을 새로 연 뒤 브라우저 개발자 도구의 Network 탭에서 `tracker.js`(Momento 는 `/momento/tracker.js`) 와 수집 요청(`/momento/collect/v1/events`) 이 200 인지 봅니다. 콘솔에 `Refused to load … Content Security Policy` 가 보이면 같은 출처가 카드 아래 표에도 떠 있을 것입니다. Momento 관리 콘솔의 `설치 진단` 탭은 수집이 서버에 실제로 도착했는지를 답합니다.
...
**방문 추적**: 기본 꺼짐. 켜면 화면 응답마다 새 nonce 로 `script-src` 를 정확히 그 스니펫만큼 넓히며 `'unsafe-inline'` 은 쓰지 않습니다. Momento 는 `/momento/*` 프록시(기본)로 두면 바깥 출처가 정책에 들어가지 않습니다. 정책 위반 신고 경로(`/api/v1/tracking/csp-report`)는 인증이 없지만 8KB 까지만 읽고 메모리의 출처 100개 목록에만 적습니다(3.10 절).
