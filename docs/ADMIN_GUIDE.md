...
GitHub Release 에서 `weekly-v0.295.0.tar.gz` 하나만 반입합니다. 자세한 검증 근거(파일 해시와 이미지 다이제스트가 왜 둘 다 필요한지)는 [README 오프라인 설치](../README.md#오프라인-설치)에 있고, 여기서는 순서대로 붙여 넣을 명령만 적습니다.

```bash
# 1. 파일이 온전히 왔는지 — 릴리즈에 적힌 SHA-256 과 비교
sha256sum weekly-v0.295.0.tar.gz

# 2. 이미지 적재. 같은 버전의 weekly:v0.295.0 이 생깁니다
gzip -dc weekly-v0.295.0.tar.gz | docker load
...
Compose 파일은 `WEEKLY_VERSION`(이미지 태그, 기본 `0.295.0`)도 읽습니다. 이것은 프로세스가 아니라 `deploy/compose.yaml`의 변수입니다.
...
사용자는 개인 설정에서 `wky_…` 키를 발급받고 원문은 발급 응답에서 한 번만 봅니다. 최대 유효일은 `보안 · 분석` 카드의 `API 키 최대 유효일`입니다. 키의 폐기는 사용자가 개인 설정에서 직접 합니다(개별 `폐기`, 또는 `모든 키 회전`으로 전부 한 번에). 관리자 화면에는 전체 키를 한 번에 폐기하는 자리가 없으므로, 사고 때는 해당 계정을 비활성으로 돌리십시오 — 비활성 계정의 키는 인증되지 않습니다. MCP 엔드포인트는 `GET/POST /mcp`(Streamable HTTP)이며 도구 목록은 [MCP.md](MCP.md)에 있습니다.
...
{"level":"INFO","msg":"Weekly started","address":":8080","version":"0.295.0"}
```
