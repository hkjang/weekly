...
`주간보고 메일 발송` 카드에서 `메일 발송 사용`, 릴레이 호스트·포트·보안·계정·보내는 주소를 저장하고 `메일 발송 시험`(`POST /api/v1/admin/settings/mail/test`, 관리자 본인 주소로 짧은 메일)을 통과시킨 뒤 사용자에게 안내합니다. 릴레이 설정은 한 번에 맞는 일이 드무니 저장한 뒤 반드시 시험 발송으로 확인하십시오 — 실패하면 릴레이가 답한 문장이 그 자리에 보입니다.

설정 키는 사내 메일 표준(MAIL-STANDARD)과 같습니다. 사내 릴레이는 **포트 25 · 인증 없음 · TLS 없음**이 흔하므로 그것이 기본값이고, 인증과 암호화는 있으면 쓰는 선택 사항입니다. 기본값은 꺼짐이라 새로 설치한 곳에서는 아무것도 나가지 않습니다.

| 키 | 기본값 | 뜻 |
|---|---|---|
| `mail.enabled` | `false` | 꺼짐이 기본. 관리자가 켭니다 |
| `mail.smtp_host` | 빈 값 | 사내 릴레이 주소(폐쇄망이면 `postra`) |
| `mail.smtp_port` | `25` | 사내 릴레이는 대개 25 |
| `mail.security` | `auto` | `auto` · `none` · `starttls` · `tls`. `auto` 는 릴레이가 STARTTLS 를 알리면 암호화하고 아니면 평문 |
| `mail.skip_tls_verify` | `false` | 릴레이 인증서가 사설일 때만 켭니다 |
| `mail.username` · `mail.password` | 빈 값 | 인증 없는 릴레이가 흔하므로 **선택 사항**. 비밀번호는 암호화 저장되며 설정 API 가 돌려주지 않습니다(화면에는 `설정됨`만) |
| `mail.from_address` · `mail.from_name` | 빈 값 · `Weekly` | 보내는 사람. 릴레이가 받아 주는 주소여야 합니다 |
| `mail.timeout_seconds` | `20` | 연결부터 본문까지 한 통에 쓰는 상한 |
| `mail.max_attempts` | `5` | 한 통을 포기하기 전 재시도 횟수(1분·4분·16분·1시간·2시간 간격) |

**계정을 쓰려면 암호화 연결이 필요합니다** — 평문 연결에는 비밀번호를 싣지 않습니다. `auto` 에서 릴레이가 STARTTLS 를 제공하지 않으면 발송이 `릴레이가 STARTTLS를 제공하지 않아 계정을 보낼 수 없습니다` 로 멈추니, 그때는 `tls` 로 바꾸거나 계정을 비우십시오. 0.301 이전 설치의 `mail.host`·`mail.port`·`mail.from` 값은 마이그레이션 033 이 새 이름으로 옮기며, 이미 켜져 있던 곳의 보안 `NONE` 은 `none` 으로 남고 켜지 않았던 곳만 `auto` 가 됩니다.
